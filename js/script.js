...
/*   var descriptions = new Array(); */
  ...
  function updateAreaList() {
    csvToArray(UserDataOrigin + "data/area_days.csv", function(tmp) {
      ...
      csvToArray(UserDataOrigin + "data/center.csv", function(tmp) {
        //ゴミ処理センターのデータを解析します。
        //表示上は現れませんが、
  ...
  function createMenuList(after_action) {
    // 備考データを読み込む
    csvToArray(UserDataOrigin + "data/remarks.csv", function(data) {
      data.shift();
      for (var i in data) {
        remarks.push(new RemarkModel(data[i]));
      }
    });
    csvToArray(UserDataOrigin + "data/description.csv", function(data) {
      data.shift();
      for (var i in data) {
        descriptions.push(new DescriptionModel(data[i]));
      }

      csvToArray(UserDataOrigin + "data/target.csv", function(data) {
